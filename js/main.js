...
///ADDING SUBMENUES TO OPETTAJALLE AND OTHERS

jQuery(document).ready(function($){
	$( "#nav2-s" ).clone().removeAttr('id').appendTo( "#opelinkit" );
	$( "#nav3-s" ).clone().appendTo( "#tietolinkit" );
	$( "#nav4-s" ).clone().appendTo( "#pelilinkit" );

	if("#subnav2") {
		$( "#nav2-s" ).clone().appendTo( "#subnav2" );
	}
	if("subnav3") {
		$( "#nav3-s" ).clone().appendTo( "#subnav3" );
	}
	if("#subnav4") {
		$( "#nav4-s" ).clone().appendTo( "#subnav4" );
	}
});
// for changing color of navigation bar 
jQuery(document).ready(function($){
    var anchor = $(".navbar-nav >li >a") 
...
// for changing color of sub navigation bar
jQuery(document).ready(function($){
    var anchor = $("#subnav2 >ul >li >a") 
	current = window.location.pathname.split('/')[3];
	
	var a = [];
  	for (var i = 0; i < anchor.length; i++) {
			 	b = anchor[i].href.split('/');
				a.push(b);
				
				if(a[a.length-1][a[a.length-1].length-1] == current) {
					
					anchor[i].className = "activeSub";
					break;
				}
				if ((a[a.length-1] != current)&& a.length == anchor[i].href.split('/').length){
					anchor[0].className = "activeSub";
				}
    }
});

jQuery(document).ready(function($){
    var anchor = $("#subnav3 >ul >li >a") 
	current = window.location.pathname.split('/')[3];
...
// for getting data from JSON highscore
...
jQuery(document).ready(function($){
	//calling json from server, as the Javascript language have something called "Croos Domain Policy", it will blocks anything that is not in the same domain
	//for example, http://www.example.com and http://example.com are 2 different domains.
  $.ajax({
            url: "/highscoreJsonCall.php",
			type: 'GET',
			async:true,
            dataType: 'json',
            ...
            success: function(result){
				...
				$(data).each(function(index) {
				  ...
				  myDivs += '<div class="highscore-img1"><img src="/themes/viku/images/highscore_'+(parseInt(index)+1).toString()+'.png"></div>';
				  ...
				  var currentLan = window.location.pathname.split('/')[1];
				  if(currentLan =="fi"){
					...
					mmuni = data[index].swe;
				  }
				  
				  myDivs += '<div class="highscore-municipality">'+mmuni+'</div>';
				  myDivs += '<div class="highscore-newline"></div>';
				  myDivs += '<div class="highscore-difficulty"><img src="/themes/viku/images/'+smallDif+'_bullet.png">'+b+'</div>';
